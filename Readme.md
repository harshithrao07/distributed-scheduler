...
|---|---:|---|
| `job-queue` | 36 | Standard-priority jobs |
| `job-queue-high` | 36 | High-priority jobs |
| `job-dlq` | 1 | Dead-letter queue |

`HIGH` priority jobs route to `job-queue-high`; all others go to `job-queue`.

Both worker queues run with Spring Kafka listener `concurrency=12` per app replica. With 36 partitions, this supports up to 3 replicas with no idle threads (12 × 3 = 36 = partition count). Across the whole consumer group, the 36-partition cap is the upper bound on concurrent workers; scaling beyond 3 replicas would either leave replicas with no partitions assigned or require bumping partition count further.
...
# Per-record acknowledgement: offset commits only after the listener method returns,
# giving at-least-once delivery anchored to PostgreSQL state.
spring.kafka.listener.ack-mode=record

# Hikari connection pool sized for 12 worker listener threads + scheduler poller +
# 2 watchdogs + DLQ scheduler + API request handlers. The default of 10 contended
# under load and added ~150ms to p95.
...
# Due-job poller (tuned aggressively after benchmarking - see Performance section)
scheduler.due-job.poll-delay-ms=100
scheduler.due-job.dispatch-retry-delay-ms=5000
scheduler.due-job.claim-limit=500
...
- 12-partition Kafka topics with listener `concurrency=12` for fan-out across worker threads on a single instance, scaling further by adding consumer-group instances
- Atomic worker-hot-path transactions: each job's `RUNNING` flip + execution-log create and `SUCCESS` flip + execution-log close each happen in **one** PostgreSQL transaction (5 → 2 transactions per job)
- HikariCP connection pool sized for the full concurrency budget (workers + scheduler + watchdogs + DLQ + API)
- At-least-once Kafka delivery via per-record `ack-mode=record`, anchored to PostgreSQL state for retry/dead-lettering
- Load-tested benchmark harness ([bench/](bench/)) with k6 + Prometheus + PostgreSQL drilldown
- Horizontally scalable: stateless app replicas behind an nginx LB share PostgreSQL/Kafka/Redis state; Redis ownership-safe locks prevent double-execution during Kafka consumer-group rebalances. Validated at 3 replicas (`docker compose up --scale app=3` or `deploy.replicas: 3`) sustaining 300 jobs/sec at e2e p99 206 ms.
...
- The architectural ceiling moves from worker concurrency to PostgreSQL CPU once you scale horizontally. At 3 replicas / 300 rps, app instances run at ~100% CPU each (~1 core) but PostgreSQL pegs at 250%+ CPU — every scheduler poll, worker state transition, dispatch claim, and watchdog query converges on the `jobs` table. The next scaling step is vertical PG (more cores) or read replicas, not more app replicas.

---

## Performance

Docker Desktop stack (app + PostgreSQL + Kafka + Redis + echo target), 36-partition topics with listener `concurrency=12` per replica, 30-connection HikariCP pool per replica, 100 ms dispatch poll, default JVM, no other tuning. Generated by [bench/run-bench.ps1](bench/run-bench.ps1).

### Headline numbers — single replica

| Sustained rate | API submission p95 | E2E p50 | E2E p95 | E2E p99 | Completion |
|---:|---:|---:|---:|---:|---|
| **200 jobs/sec** | **5.3 ms** | **161 ms** | **271 ms** | **316 ms** | **12,001 / 12,001** |
| **300 jobs/sec** | **5.9 ms** | **363 ms** | **708 ms** | **929 ms** | **18,000 / 18,000** |
| 400 jobs/sec (overload) | 5.9 ms | 3.38 s | 5.61 s | 6.08 s | **24,001 / 24,001** |

### Headline numbers — 3 replicas behind nginx LB

| Sustained rate | API submission p95 | E2E p50 | E2E p95 | E2E p99 | Completion |
|---:|---:|---:|---:|---:|---|
| **300 jobs/sec** | **7.5 ms** | **84 ms** | **169 ms** | **206 ms** | **18,001 / 18,001** |

3 replicas at 300 rps is **4.5× better** on p99 than single-replica at the same rate (929 ms → 206 ms), with the same 100% completion. Architectural ceiling at this scale is **PostgreSQL CPU** (sustains ~250% CPU during the 3-replica run, observed via `docker stats`), not the app tier — the dispatch query, worker writes, and cron scheduling all converge on the `jobs` table.

Beyond this rate the system stays lossless (verified at 600 jobs/sec × 36,000 submissions, 100% completion) but PG saturates and end-to-end latency degrades sharply — past sustainable steady state.

E2E latency = `completed_at − created_at` (PostgreSQL), measured per-run via [bench/measure-e2e.sql](bench/measure-e2e.sql). Worker handler itself averages ~6 ms (Webhook + Redis lock acquire/release + 2 atomic DB transactions). With a 100 ms dispatch poll, the residual e2e gap above the handler is roughly: ~50 ms avg poll wait + Kafka roundtrip + Redis lock + 2 DB transactions + connection-pool acquisition.

The tight p95–p99 spread at 200 and 300 rps (271 → 316 ms, 708 → 929 ms) means the system has no nasty tail behavior — when it's slow, it's slow consistently, not erratically.

### How the numbers got here

| Stage | Sustained rate | E2E p95 | E2E p99 |
|---|---:|---:|---:|
| Original config (5s poll, claim 100, 1 listener thread) | ~30 jobs/s | 789 ms | 857 ms |
| + Tuned dispatcher (500 ms poll, claim 500) and Kafka 12 partitions / `concurrency=12` | 200 jobs/s* | 1.40 s | 1.59 s |
| + Race fix: removed `markDispatchSucceeded` callback that overwrote worker `SUCCESS` | 200 jobs/s (now lossless) | 1.40 s | 1.59 s |
| + Hikari pool 30 + atomic 2-transaction worker hot path | 200 jobs/s | 1.20 s | 1.32 s |
| + Dispatch poll 500 ms → 100 ms | **200 jobs/s** | **271 ms** | **316 ms** |
| + Same config, 300 rps (single-replica headline) | **300 jobs/s** | **708 ms** | **929 ms** |
| + Horizontally scaled to 3 replicas (Kafka 36 partitions, nginx LB) | **300 jobs/s** | **169 ms** | **206 ms** |

\* Before the race fix landed, completion at 200 rps was 5,917/6,001 = 98.6%. The 84 stuck jobs all had execution_log `SUCCESS` but job status `QUEUED` — surfaced only after raising listener concurrency from 1 → 12, which made the producer-callback race materialize at high rate.

### Horizontal scaling validation

3 app replicas (`scheduler-app-1/2/3`) sit behind an nginx load balancer with shared PostgreSQL, Kafka (consumer-group rebalanced across 36 partitions), and Redis. Stateless workers + Redis ownership-safe locks + Kafka offset commits guarantee no double-execution during rebalance.

Live `docker stats` during the 3-replica @ 300 rps run identifies the actual bottleneck:

| Container | Peak CPU during 300 rps run | What it tells us |
|---|---:|---|
| **scheduler-postgres** | **214–254%** | Burning 2–3 cores. The architectural ceiling at this scale. |
| scheduler-app-1/2/3 | 88–133% each | Healthy headroom (~1 core each). Worker tier not saturated. |
| scheduler-kafka | 50–101% | One core busy with 36-partition broker work. |
| scheduler-redis | 12–20% | Cruising. |
| scheduler-lb (nginx) | 11–14% | Adds ~5–15 ms per request, not the dominant cost. |

The bottleneck is **PostgreSQL CPU**, not the app tier or Kafka. With 3 schedulers polling every 100 ms, 36 worker threads writing per-job state transitions, watchdogs, and dispatch claim queries (`FOR UPDATE SKIP LOCKED`) all converging on the `jobs` table, the next scaling step is vertical PG (more cores) or read replicas / sharding — not adding more app replicas.

### Kafka outage survival

[bench/recovery-test.ps1](bench/recovery-test.ps1) stops the Kafka container, submits jobs through the REST API, restarts Kafka, and measures how long it takes the backlog to drain.
...
# 2. Run the benchmark (defaults: 50 rps for 60s)
.\bench\run-bench.ps1 -Rps 200 -Duration 60s

# 3. Kafka outage survival test
.\bench\recovery-test.ps1 -JobCount 200 -OutageSeconds 60
```

Each run writes a JSON summary to `bench/results/<runId>.json`. See [bench/README.md](bench/README.md) for the full harness usage and resume-line templates.
