...
|---|---:|---|
| `job-queue` | 12 | Standard-priority jobs |
| `job-queue-high` | 12 | High-priority jobs |
| `job-dlq` | 1 | Dead-letter queue |

`HIGH` priority jobs route to `job-queue-high`; all others go to `job-queue`.

Both worker queues run with Spring Kafka listener `concurrency=12`, so a single app instance fans 12 worker threads across the 12 partitions for parallel execution. Across multiple instances, Kafka's consumer group rebalances the partitions; the 12-partition cap is the upper bound on concurrent workers across the whole consumer group.
...
Single-node Docker Desktop stack (app + PostgreSQL + Kafka + Redis + echo target), 12-partition topics, listener `concurrency=12`, 30-connection HikariCP pool, 100 ms dispatch poll, default JVM, no other tuning. Generated by [bench/run-bench.ps1](bench/run-bench.ps1).
...
| + Same config, 300 rps (new comfortable headline) | **300 jobs/s** | **708 ms** | **929 ms** |
